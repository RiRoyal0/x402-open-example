...
const PROTECTED_API_URL = "http://localhost:4021/weather";
...
async function main() {
  ...
  try {
    ...
    const signer = { svm: payer };
    console.log('  → Network:', NETWORK);
    console.log('  → Payer address:', signer.svm.address.slice(0, 4) + '...' + signer.svm.address.slice(-4));
    ...
    try {
      if (paymentResponseHeader) {
        ...
        console.log('  ⚠ No payment response to decode (header missing)');
      }
    } catch (decodeError) {
      decodedPaymentResponse = null;
      console.log('  ⚠ Failed to decode payment response');
    }
